...
const mulai = async (sesi, conn = new WAConnection()) => {
     ...
     conn.on('contacts-received', async () => {
          ...
          fs.writeFileSync('./src/sessions/sessions.json', JSON.stringify(authInfo, null, 2))
          delay(1000)
          status.stop()
          starting.stop()
          if (!IsShouldmessage) {
     ...
     fs.existsSync('./src/sessions/sessions.json') && conn.loadAuthInfo('./src/sessions/sessions.json')
